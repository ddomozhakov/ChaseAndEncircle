...
import chase.logic.Enemy;
import chase.logic.GameField;
import chase.logic.Player;
import game.logic.Parameters;
...
public class Program {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < jArgs.enemiesCount; i++) {
            enemies[i] = new Enemy(gameField.getEnemiesCoord()[i][0], gameField.getEnemiesCoord()[i][1],
                    gameField, player);
        }
        ...
        while (checkWin == 0 && movement != 9) {
            ...
            for (int i = 0; i < jArgs.enemiesCount; i++) {
                if (parameters.isDebug()) {
                    do {
                        render.Rendering();
                        System.out.printf("Enemy %d/%d (%d, %d): ", i + 1, jArgs.enemiesCount,
                                gameField.getEnemiesCoord()[i][0], gameField.getEnemiesCoord()[i][1]);
                    } while (userIn(in) != 8);
                }
